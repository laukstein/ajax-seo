...
$js  = $debug ? '<script src="' . assets . "script.js#" . (strlen($path) ? $path : '/') . '"></script>' : '<script>!function(t){"use strict";"function"==typeof define&&define.amd?define(["as"],t):"object"==typeof module&&module.exports?module.exports=t():window.as=t()}(function(){"use strict";var t,e,r,s,a=window,n=document,o=history,i=location,l={classList:"classList"in n.documentElement,click:"click"in n.documentElement,eventListener:!!n.addEventListener,touch:a.hasOwnProperty("ontouchstart"),valid:function(t){try{return t()}catch(e){return l.valid.error.e=e,l.valid.error}}},c={bar:n.getElementById("bar"),focusin:n.getElementById("focusin"),focusout:n.getElementById("focusout"),reset:n.getElementById("reset"),nav:n.getElementById("nav"),status:n.getElementById("status"),output:n.getElementById("output")},u={toggleClass:function(t,e){if(t)if(l.classList)t.classList.toggle(e);else{var r=t.className.split(" "),s=r.indexOf(e);s>=0?r.splice(s,1):r.push(e),t.className=r.join(" ")}},toToggle:function(t){this.toggleClass(n.body.parentElement,"noscroll"),this.toggleClass(c.status,"expand"),t&&c.focusin&&setTimeout(function(){c.focusin.focus()},0)},toFocus:function(){n.activeElement!==c.focusin&&this.toToggle(!0)}},f={version:4.3,analytics:"' . ga . '",origin:function(){var t=n.currentScript||function(){var t=n.getElementsByTagName("script");return t[t.length-1]}(),e=t.src.split("#")[1]||"/ajax-seo",r=new RegExp("("+e+")(.*)$");return n.URL.replace(r,"$1")}(),url:n.URL,title:n.title,activeElement:function(){var t,e=n.querySelectorAll?n.querySelectorAll("[href]"):[];for(t=0;t<e.length;t+=1)if(e[t].href.toUpperCase()===n.URL.toUpperCase())return e[t];return null}(),error:c.status&&(l.classList?c.status.classList.contains("status-error"):new RegExp("(^|\\s)status-error(\\s|$)").test(c.status.className))};if(l.valid.error={e:null},f.analytics&&(a.ga=function(){ga.q=ga.q||[],ga.q.push(arguments)},ga("create",f.analytics,"' . ga_domain . '"),ga("send","pageview")),c.bar&&l.eventListener&&(c.bar.addEventListener("focus",function(){u.toToggle(!0)},!0),c.bar.addEventListener("click",function(){u.toFocus()},!0),c.nav&&c.nav.addEventListener("click",function(){u.toToggle(!0)},!0),c.focusout&&c.focusout.addEventListener("focus",function(){u.toToggle(!1)},!0),c.reset&&c.reset.addEventListener("click",function(){u.toToggle(!0)},!0)),!o.pushState||!l.classList||!l.eventListener)throw new Error("Browser legacy: History API not supported");if(!c.output)throw new Error("Layout issue: missing elements");return s={filter:function(t,e){return t?(t=t.replace(/#.*$/,""),e?t:t.toLowerCase()):void 0},reset:function(){e&&clearTimeout(e),c.status&&c.status.classList.contains("status-start")&&c.status.classList.add("status-done")},click:function(t){if(t)if(l.click)t.click();else{var e;e=n.createEvent("MouseEvents"),e.initEvent("click",!0,!0),t.dispatchEvent(e)}},nav:{nodeList:c.nav?Array.from?Array.from("a"):[].slice.call(c.nav.querySelectorAll("a")):null,activeElement:function(){if(s.nav.nodeList){var t;for(t=0;t<s.nav.nodeList.length;t+=1)if(s.filter(s.nav.nodeList[t].href)===f.url)return s.nav.nodeList[t]}return null}},update:function(t,e,a){if(t){f.analytics&&ga("send","pageview",{page:f.url}),e?s.reset():r.abort(),s.nav.nodeList&&(c.focus=c.nav.querySelector(".focus"),c.active=c.nav.querySelector(".active"),c.error=c.nav.querySelector(".error"),c.focus&&c.focus.classList.remove("focus"),c.active&&c.active.classList.remove("active"),c.error&&c.error.classList.remove("error")),n.activeElement&&"BODY"===n.activeElement.tagName&&n.activeElement.blur(),f.url=s.filter(n.URL),a=a||s.nav.activeElement(),a&&(a.focus(),a.classList.add(f.error?"error":"active"),f.error&&a.classList.add("x-error")),f.error?(c.status.classList.add("error"),c.status.classList.add("status-error")):(c.status.classList.remove("error"),c.status.classList.remove("status-error")),n.title=f.title=t.title;var o=n.scrollingElement||n.documentElement.scrollTop||n.body;o.scrollTop=0,c.output.innerHTML=t.content,i.hash&&i.replace(f.url+i.hash)}},retry:!1,popstate:function(t){if(!(i.hash&&s.filter(f.url)===s.filter(n.URL)||f.url&&f.url.indexOf("#")>-1)){var e,r=t.state;s.reset(),s.retry=!r,f.error=r&&r.error||!1,r||(f.url=s.filter(n.URL),e=s.nav.activeElement(),s.click(e)),n.activeElement.blur(),s.update(r,!1,e)}},loadstart:function(){c.status&&(c.status.classList.remove("status-done"),c.status.classList.remove("status-start"),e&&clearTimeout(e),e=setTimeout(function(){c.status.classList.add("status-start")},0))},callback:function(t){f.error=t.error||!1,o.replaceState(t,t.title,null),s.update(t,!0)},load:function(){var e=this.response;e=l.valid(function(){return JSON.parse(e)}),t&&clearTimeout(t),s.callback(e===l.valid.error?{error:!0,title:"Server error",content:"<h1>Whoops...</h1><p>Experienced server error. Try to <a class=x-error href="+f.url+">reload</a>"+(f.url===f.origin?"":" or head to <a href="+f.origin+">home page</a>")+"."}:e)},closest:function(t,e){if(!t||!e)return null;if(t.closest)return t.closest(e);for(var r=t.matches||t.webkitMatchesSelector||t.msMatchesSelector;t&&1===t.nodeType;){if(r.call(t,e))return t;t=t.parentNode}return null},resetStatus:function(t){c.status&&(!c.status.classList.contains("status-error")||t&&f.error||c.status.classList.remove("status-error"),c.status.classList.contains("status-done")&&(c.status.classList.remove("status-start"),c.status.classList.remove("status-done")))},listener:function(e){if(e){var a=e.target,i=new RegExp("^"+f.origin+"($|#|/.{1,}).*","i"),l={};if(!a)return;if("A"!==a.tagName&&(a=s.closest(a,"a[href]")),!a||"A"!==a.tagName||!a.hasAttribute("href")||!i.test(a.href))return;if(f.url=a.href.toLowerCase().replace(/(\/)+(?=\1)/g,"").replace(/(^https?:(\/))/,"$1/").replace(/\/$/,""),l.attr=s.filter(f.url,!0),l.address=s.filter(n.URL),l.attr===l.address&&f.url.indexOf("#")>-1)return;if(e.preventDefault(),a.blur(),f.activeElement=a,f.activeNav=a.parentNode===c.nav,!s.retry&&f.activeNav&&(f.error=f.activeElement.classList.contains("x-error")),f.title=f.activeElement.innerText.replace(/\n/,"")||f.activeElement.textContent,f.error&&l.address===n.URL?o.replaceState(null,f.title,f.url):f.error||f.url===n.URL||o.pushState(null,f.title,f.url),!f.error&&!s.retry&&l.attr===l.address||f.activeNav&&f.activeElement.classList.contains("focus"))return;t&&clearTimeout(t),t=setTimeout(function(){n.title=f.title},3),s.resetStatus(),s.nav.nodeList&&(f.error&&f.activeNav&&(f.activeElement.classList.remove("x-error"),f.activeElement.classList.remove("error")),c.focus=c.nav.querySelector(".focus"),c.focus&&c.focus.classList.remove("focus")),f.activeNav&&f.activeElement.classList.add("focus"),r.abort(),r.open("GET",f.origin+"/api"+l.attr.replace(new RegExp("^"+f.origin,"i"),"")),f.error&&r.setRequestHeader("If-Modified-Since","Sat, 1 Jan 2000 00:00:00 GMT"),r.send()}},init:function(){c.status&&c.status.addEventListener("transitionend",s.resetStatus,!0),setTimeout(function(){a.onpopstate=s.popstate},150),o.replaceState({error:f.error,title:f.title,content:c.output.innerHTML},f.title,f.url),r=new XMLHttpRequest,r.onloadstart=s.loadstart,r.onload=s.load,r.onabort=s.reset,n.addEventListener(l.touch?"touchstart":"click",s.listener,!0)}},f.analytics||delete f.analytics,s.init(),f});
</script>';
